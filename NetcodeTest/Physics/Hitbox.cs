...
namespace NetcodeTest.Physics
{
    ...
    public class Hitbox
    {
        ...
        {
            double combined_radsq = radius + hitbox.radius;
    ...
    public class HitboxCircle : Hitbox
    {
        public HitboxCircle(float arg_radius)
        {
            ...
            size = new Vector2((float)radius, (float)radius);
        }
        ...
        public override Intersection Intersect(Vector2 point)
        {
            ...
            sect.surface_normal = Util.AngleTo(pos, point);
            sect.position = point;
            return sect;
        }
        ...
        public override Intersection Intersect(Hitbox hitbox)
        {
            ...
            if (hitbox is HitboxCircle)
            {
                ...
                sect.surface_normal = Util.AngleTo(pos - hitbox.pos);
    ...
    public class HitboxPolygon : Hitbox
    {
        ...
        public HitboxPolygon(Vector2[] arg_corners)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                float segment_angle = Util.AngleTo(segment_p2 - segment_p1);
                segment_normals[i] = Util.WrapAngle(segment_angle - MathHelper.PiOver2);
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                corner_normals[i] = Util.WrapAngle(c_normal);
            }
        }
        ...
        public override Intersection Intersect(Vector2 arg_point)
        {
            ...
            Vector2 point = Util.Rotate(arg_point - pos, -angle);
        ...
        public Intersection CalculateIntersection(Vector2 point)
        {
            ...
            // first shift the point into the reference frame of the hitbox
            point = Util.Rotate(point - pos, -angle);
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                float t = Util.Clamp(Vector2.Dot(point - p1, pd) / pd.LengthSquared(), 0.0f, 1.0f);
            ...
            sect.surface_normal = Util.WrapAngle(segment_normals[best_segment] + angle);
        ...
        public Intersection IntersectCircle(HitboxCircle hitbox)
        {
            ...
            Vector2 point = Util.Rotate(hitbox.pos - pos, -angle);
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (Vector2.DistanceSquared(point, corners[i]) < hitbox.radius_sq)
                {
                    ...
                    sect.position = pos + Util.Rotate(corners[i], angle);
        ...
        Intersection IntersectPolygonHalf(HitboxPolygon hitbox)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                Vector2 point = pos + Util.Rotate(corners[i], angle);
                ...
                if (sect != null)
                {
                    ...
                    if (best_sect == null || sect.overlap > best_sect.overlap)
                    {
                        ...
                        float delta = Util.AngleDelta(corner_normal + MathHelper.Pi, sect.surface_normal);
                        sect.surface_normal = Util.WrapAngle(sect.surface_normal + delta / 2.0f);
        ...
        public Intersection IntersectPolygon(HitboxPolygon hitbox)
        {
            ...
            if (next_sect != null)
            {
                next_sect.surface_normal = Util.WrapAngle(next_sect.surface_normal + MathHelper.Pi);
            }
        ...
        public static HitboxPolygon Square( float size )
        {
            return new HitboxPolygon(
                new Vector2[]
                {
